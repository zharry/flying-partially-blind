...
class GridVisualizer:
    ...
    def _setup_plot(self):
        ...
        self.fig, self.ax = plt.subplots(figsize=(10, 10), facecolor='#1a1a2e')
        ...
        for i in range(self.grid_width + 1):
            self.ax.axvline(x=i - 0.5, color='#0f3460', linewidth=0.5, alpha=0.7)
        for i in range(self.grid_height + 1):
            self.ax.axhline(y=i - 0.5, color='#0f3460', linewidth=0.5, alpha=0.7)
        
        # Draw obstacles
        for obs in self.obstacles:
            obstacle_rect = patches.FancyBboxPatch(
                (obs.x - 0.4, obs.y - 0.4), 0.8, 0.8,
